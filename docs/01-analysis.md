...
Статус: **ЭТАП 4 РЕАЛИЗОВАН** (2026-09-22): версионирование маршрутов и редактор на карте. Решения заказчика — раздел 11, ход работ — разделы 12, 14–17.
Дата: 2026-09-21
...
## 16. Исправление: путь маршрутов по дорогам (2026-09-22)

**Проблема, найденная заказчиком.** Маршруты на карте были построены не по дороге: линия
соединяла остановки напрямую. Это касалось не только внешнего вида — по прямой считались
остаток пути до остановки в GPS-прогнозе и отклонение от маршрута, поэтому автобус на изгибе
улицы мог выглядеть сошедшим с маршрута.

**Решение.** Геометрия маршрута запрашивается у OSRM-совместимого сервиса и сохраняется:
`routes.path` (полилиния), `routes.path_distance_m`, `routes.path_source` и
`route_stops.road_distance_m` (расстояние до остановки по дороге). Запрос выполняется только
при создании или изменении маршрута, при сиде и по кнопке администратора — не при отрисовке.

Доменная логика переработана: введено понятие `RoutePath` (полилиния плюс положение остановок
на ней), а проекция позиции и расчёт времени работают с ней одинаково — и для пути по дорогам,
и для прямых линий как отката. Прежние функции сохранены как обёртки, поэтому старые вызовы
и тесты продолжают работать.

| Решение | Причина |
|---------|---------|
| Хранить геометрию в базе, а не запрашивать при рендере | Страница не должна зависеть от внешнего сервиса; маршрут меняется редко |
| Адрес сервиса в `ROUTING_URL`, по умолчанию публичный OSRM | Для продакшена нужен свой инстанс: у демо-сервера нет гарантий |
| Откат на прямые линии при недоступности сервиса | Маршрут остаётся рабочим, а в списке видно пометку «прямые линии» |
| Расстояние до остановки брать из ответа маршрутизатора | Точнее, чем измерять по сохранённой полилинии |

**Проверено.** Все шесть маршрутов получили геометрию: №1 утром 446 точек и 18,7 км по дорогам
против примерно 11 км по прямой. Расстояния по остановкам сохранены (0, 777, 1920, 2685, 6665,
8981, 12002, 18736 м). На карте маршрута и в мониторинге линии идут по улицам, остановки лежат
на линии. Тестов домена 103, из них 14 новых на геометрию: они проверяют, что объезд считается
по дороге, а не по прямой.

Оценка длины в предложениях новых маршрутов осталась по прямой и помечена в интерфейсе как
предварительная: точный путь строится при создании черновика.

---

## 17. Этап 4: версионирование маршрутов и редактор на карте (2026-09-22)

### Версионирование

**Проблема (CONFLICT #10 из раздела 10).** При изменении маршрута прошлое переписывалось:
завершённые рейсы начинали ссылаться на новый порядок остановок, и разбивка загрузки
по остановкам показывала то, чего не было. В MVP это было сознательно отложено.

**Решение.** Форма маршрута вынесена в `route_versions`: порядок остановок, смещения времени
и геометрия по дорогам. `route_stops` принадлежат версии, `trips.route_version_id` закрепляет
рейс за формой, по которой он фактически ехал, `routes.current_version_id` указывает на
действующую форму.

| Решение | Причина |
|---------|---------|
| Версия создаётся только при изменении формы | Правка названия или цвета не должна плодить версии |
| Запланированные рейсы переходят на новую версию | Они ещё не выполнялись и поедут по новой форме |
| Завершённые и выполняемые остаются на своей | Это и есть сохранение истории |
| Геометрия принадлежит версии, а не маршруту | У каждой формы свой путь по дорогам |
| Старая версия не удаляется, пока на ней есть рейсы | Иначе история снова потеряется |
| Миграция: колонка пустая → перенос → NOT NULL | Прямое добавление NOT NULL упало бы на существующих строках |

**Проверено на живых данных.** Маршрут №1 (утро): добавлена остановка в Гафурове — создана
версия 2 (9 остановок, 29,3 км), 33 завершённых рейса остались на версии 1 (8 остановок,
18,7 км), 15 запланированных перешли на версию 2. Все 264 отметки водителя по остановкам
по-прежнему сопоставляются со своей формой, ни одна не потерялась. Возврат к прежнему
маршруту создал версию 3, завершённые рейсы остались на версии 1.

### Редактор маршрута на карте

Маршрут собирается кликами: нажатие на пустое место карты создаёт остановку (название
вводится в появившейся панели), нажатие на серую точку добавляет существующую остановку.
Точки маршрута показаны нумерованными маркерами в порядке движения.

Кнопка «Показать путь по дорогам» обращается к маршрутизатору через `POST /api/v1/routing/preview`
и рисует настоящий путь, а кнопка «Подставить время» заполняет смещения по фактическим
расстояниям. Запрос выполняется только по нажатию администратора — правило «никакой
маршрутизации при отрисовке» сохранено.

Новые остановки создаются в справочнике только при сохранении маршрута: пока администратор
не нажал «Сохранить», в базе ничего не появляется.

**Проверено.** Точка поставлена кликом по карте (40.25490, 69.74430), предпросмотр показал
29,3 км вместо 18,7, автоподстановка заполнила смещения 0/3/7/10/22/29/39/58/88, сохранение
создало версию с новой остановкой и геометрией.

---

## 18. Parking lot (этап 5+)
