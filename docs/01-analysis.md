...
Статус: **MVP РЕАЛИЗОВАН** (2026-09-21). Решения заказчика — раздел 11, выполненные задачи — раздел 12.
Дата: 2026-09-21
...
## 13. Parking lot (этап 2+)

- GPS: `vehicle_positions`, транспорт на карте, отклонение от маршрута, «нет транспорта на маршруте».
- Push-уведомления (web-push для PWA, затем нативные).
- Supabase Realtime вместо опроса.
- Версионирование маршрутов (`route_versions`) для корректной истории.
- Прогноз спроса и автоматические предложения новых маршрутов по адресам сотрудников (кластеризация домашних координат).
